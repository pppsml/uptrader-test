...
let ProjectService = class ProjectService {
    ...
    async create(createProjectDto) {
        try {
            if (!createProjectDto.name)
                throw new common_1.BadRequestException('"name" field is required');
    ...
    async getOne(projectId) {
        try {
            ...
            if (!project)
                throw new common_1.NotFoundException(`Project with id "${projectId}" does now exist`);
    ...
    async delete(projectId) {
        try {
            if (!projectId)
                throw new common_1.BadRequestException('"projectId" query parameter is required');
            const project = await (await this.projectModel.findByIdAndDelete(projectId)).populate('tasks');
